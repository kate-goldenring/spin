...
impl Prompter {
    pub fn new(plugin_name: &str, plugin_license: &str, plugins_repo_url: Url, source_url: &str) -> Result<Self> {
        Ok(Self {
    ...
    fn print_prompt(&self) {
        println!("Installing plugin {} with license {} from {}\n", self.plugin_name, self.plugin_license, self.source_url);
        println!("For more information, reference the plugin metadata at {}\n", self.plugin_repo_url);
    ...
    fn parse_response(&self, resp: &mut String) -> bool {
...
mod test {
    ...
    fn test_parse_response() {
        let p = Prompter::new("best-plugin", "MIT", Url::parse("www.spin.com/spin-plugins/best-plugin").unwrap(), "www.example.com").unwrap();
